...
async def ai_chat_req(history, user_text):
    sys="Siz BilimChallenge botining yordamchi AI sisiz. O'zbek tilida qisqa va foydali javob bering."
    try:
        prompt=sys+"\n\n"
        for h in history[-6:]:
...
async def cmd_start(message: types.Message):
    ...
    await message.answer(
        ...
        "🔥 Streak yig'ing\n🏆 Global reyting\n🎓 IELTS — AI bilan\n🤖 AI Chat\n\n"
        "Boshlash uchun <b>Savol olish</b>!",
...
async def get_question_start(message: types.Message, state: FSMContext):
    ...
    await message.answer("📂 <b>Kategoriya tanlang:</b>",parse_mode="HTML",reply_markup=InlineKeyboardMarkup(inline_keyboard=buttons))
...
async def send_question(message,user_id,state,category,mode="all"):
    ...
    if q_type=="test":
        ...
        active_timers[user_id]=asyncio.create_task(question_timeout(user_id,q_id,sent.message_id,message.chat.id,coins,state,q_time))
    else:
...
async def question_timeout(user_id,q_id,msg_id,chat_id,coins,state,q_time=30):
    ...
    try:
        await bot.send_message(chat_id,f"⏰ <b>Vaqt tugadi!</b>\n❌ -{penalty} coin (45%)",parse_mode="HTML",
            reply_markup=InlineKeyboardMarkup(inline_keyboard=[[InlineKeyboardButton(text="➡️ Keyingi savol",callback_data=f"next_{mode}_{cat}")]]))
    except: pass
...
async def handle_test_answer(callback: types.CallbackQuery, state: FSMContext):
    ...
    await callback.message.answer(text,parse_mode="HTML",reply_markup=InlineKeyboardMarkup(inline_keyboard=[[InlineKeyboardButton(text="➡️ Keyingi savol",callback_data=f"next_{mode}_{cat2}")]]))
    await callback.answer()
...
async def handle_open_answer(message: types.Message, state: FSMContext):
    ...
    await message.answer(text,parse_mode="HTML",reply_markup=InlineKeyboardMarkup(inline_keyboard=[[InlineKeyboardButton(text="➡️ Keyingi savol",callback_data=f"next_{mode}_{cat}")]]))
    await state.clear()
...
async def skip_open(callback: types.CallbackQuery, state: FSMContext):
    ...
    await callback.message.answer("⏭ O'tkazib yuborildi.",reply_markup=InlineKeyboardMarkup(inline_keyboard=[[InlineKeyboardButton(text="➡️ Keyingi savol",callback_data=f"next_{mode}_{cat}")]]))
    await callback.answer()
...
async def handle_premium(message: types.Message, state: FSMContext):
    ...
    if is_correct:
        ...
        await message.answer(text,parse_mode="HTML",reply_markup=InlineKeyboardMarkup(inline_keyboard=[[InlineKeyboardButton(text="⭐ Keyingi premium savol",callback_data=f"premium_start_{cat}")]]))
        await state.clear()
    else:
        ...
        if remaining>0:
            ...
            await message.answer("😔 3 ta urinish tugadi.",reply_markup=InlineKeyboardMarkup(inline_keyboard=[[InlineKeyboardButton(text="⭐ Keyingi premium savol",callback_data=f"premium_start_{cat}")]]))
            await state.clear()
...
async def skip_premium(callback: types.CallbackQuery, state: FSMContext):
    ...
    await callback.message.answer("⏭ O'tkazib yuborildi.",reply_markup=InlineKeyboardMarkup(inline_keyboard=[[InlineKeyboardButton(text="⭐ Keyingi premium savol",callback_data=f"premium_start_{cat}")]]))
    await callback.answer()
...
async def ielts_section(callback: types.CallbackQuery, state: FSMContext):
    ...
    await callback.message.answer("👆 Javob yuboring:",reply_markup=InlineKeyboardMarkup(inline_keyboard=[[InlineKeyboardButton(text="⏭ O'tkazib yuborish",callback_data=f"skip_ielts_{section}")]]))
    await callback.answer()
...
async def handle_ielts(message: types.Message, state: FSMContext):
    ...
    if section=="reading":
        ...
        result+=f"\n\n💰 +{earned} coin"
        await message.answer(result,parse_mode="HTML",reply_markup=InlineKeyboardMarkup(inline_keyboard=[[InlineKeyboardButton(text=f"➡️ Keyingi {section.upper()}",callback_data=f"ielts_{section}")]]))
    ...
    await message.answer(f"🤖 <b>AI Tahlil:</b>\n\n{analysis}{st}\n\n💰 +{earned} coin (max {coins})",
        parse_mode="HTML",reply_markup=InlineKeyboardMarkup(inline_keyboard=[[InlineKeyboardButton(text=f"➡️ Keyingi {section.upper()}",callback_data=f"ielts_{section}")]]))
    await state.clear()
...
async def skip_ielts(callback: types.CallbackQuery, state: FSMContext):
    ...
    await callback.message.answer("⏭ O'tkazib yuborildi.",reply_markup=InlineKeyboardMarkup(inline_keyboard=[[InlineKeyboardButton(text=f"➡️ Keyingi {section.upper()}",callback_data=f"ielts_{section}")]]))
    await callback.answer()
...
async def show_profile(message: types.Message):
    ...
    await message.answer(
        ...
        f"🎯 Aniqlik: <b>{accuracy}%</b>\n📅 Qo'shilgan: <b>{join_date[:10]}</b>",parse_mode="HTML")

@dp.message(F.text=="ℹ️ Yordam")
async def show_help(message: types.Message):
    await message.answer("ℹ️ <b>BilimChallenge — Yordam</b>\n\n🎯 Savol olish — kategoriya va tur\n🎓 IELTS — AI bilan mashq\n🤖 AI Chat — coinlar evaziga\n🏆 Reyting — top 10\n\n💰 To'g'ri — coin\n❌ Noto'g'ri — 30% jarima\n⏰ Vaqt — 45% jarima\n🔥x3=1.5  🔥🔥x5=2.0  🔥🔥🔥x10=3.0\n🟢Oson=30s  🟡O'rta=60s  🔴Qiyin=90s",parse_mode="HTML")
...
async def show_feedbacks(message: types.Message):
    ...
    for fb in feedbacks[:10]:
        ...
        await message.answer(f"{read_icon} <b>#{fb_id}</b> — {fname} ({uname})\n📅 {fb_date[:10]}\n\n{fb_text}",
            parse_mode="HTML",reply_markup=InlineKeyboardMarkup(inline_keyboard=[[InlineKeyboardButton(text="🗑 O'chirish",callback_data=f"del_fb_{fb_id}")]]))
...
async def manage_categories(message: types.Message):
    ...
    kb=InlineKeyboardMarkup(inline_keyboard=[[InlineKeyboardButton(text="➕ Yangi kategoriya",callback_data="add_category")],[InlineKeyboardButton(text="🗑 O'chirish",callback_data="del_category_list")]])
