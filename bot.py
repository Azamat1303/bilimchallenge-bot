...
from config import BOT_TOKEN, ADMIN_IDS, PENALTY_PERCENT, TIMEOUT_PENALTY, STREAK_BONUSES, GROQ_API_KEY, GROQ_MODEL, GEMINI_API_KEY, WEBSITE_URL
...
MENU_TEXTS = {
    ...
    "🤖 AI Chat","👥 Guruhga ulash","🌐 Websayt","⚙️ Admin Panel",
    ...
    "📥 Savollarni import qilish","📝 So'z qo'shish","📋 So'zlar ro'yxati",
    "➕ Savol yuborish","📝 Hisobot yozish",
...
def main_menu(uid):
    ...
    if WEBSITE_URL:
        b.append([KeyboardButton(text="🌐 Websayt")])
    if is_admin(uid):
...
def admin_menu():
    b = [[KeyboardButton(text="🟢 Live Savol tashlash"), KeyboardButton(text="➕ Savol qo'shish")],
         [KeyboardButton(text="📝 So'z qo'shish"),       KeyboardButton(text="📋 So'zlar ro'yxati")],
...
async def groq_call(messages, max_tokens=1000):
    """Gemini API orqali AI so'rov"""
    # Groq formatini Gemini formatiga o'girish
    system_text = ""
    parts_list = []
    for msg in messages:
        if msg["role"] == "system":
            ...
            parts_list.append({"role": "user", "parts": [{"text": msg["content"]}]})
        elif msg["role"] == "assistant":
            parts_list.append({"role": "model", "parts": [{"text": msg["content"]}]})

    # Oxirgi user xabar
    if not parts_list:
        # Oddiy prompt bo'lsa
        text_content = messages[-1]["content"] if messages else ""
        parts_list = [{"role": "user", "parts": [{"text": (system_text + "\n\n" + text_content).strip()}]}]
    elif system_text and parts_list[0]["role"] == "user":
        first_text = parts_list[0]["parts"][0]["text"]
        parts_list[0]["parts"][0]["text"] = system_text + "\n\n" + first_text
        ...
        try:
            ...
            async with aiohttp.ClientSession() as s:
                async with s.post(
                    ...
                    if r.status != 200:
                        text = await r.text()
                        logging.warning(f"Gemini {model} HTTP {r.status}: {text[:200]}")
                        continue
                    d = await r.json()
                    if "error" in d:
                        logging.warning(f"Gemini {model} error: {d['error']}")
                        continue
                    candidates = d.get("candidates", [])
                    if candidates:
                        result = candidates[0]["content"]["parts"][0]["text"]
                        logging.info(f"Gemini {model} OK: {len(result)} chars")
                        return result
        except asyncio.TimeoutError:
            logging.warning(f"Gemini {model} timeout")
        except Exception as e:
            logging.warning(f"Gemini {model} exception: {e}")
    logging.error("Barcha Gemini modellari ishlamadi!")
    return None
...
async def ai_audio_req(file_id, prompt_text):
    """Gemini orqali audio faylni tahlil qilish"""
    import base64
    try:
        file = await bot.get_file(file_id)
        file_bytes = await bot.download_file(file.file_path)
        audio_b64 = base64.b64encode(file_bytes.read()).decode("utf-8")
        mime_type = "audio/ogg"
        for model in GEMINI_MODELS:
            try:
                url = f"https://generativelanguage.googleapis.com/v1beta/models/{model}:generateContent?key={GEMINI_API_KEY}"
                ...
                async with aiohttp.ClientSession() as s:
                    async with s.post(url, headers={"Content-Type": "application/json"},
                        ...
                        if r.status != 200:
                            text = await r.text()
                            logging.warning(f"Gemini audio {model} HTTP {r.status}: {text[:200]}")
                            continue
                        d = await r.json()
                        candidates = d.get("candidates", [])
                        if candidates:
                ...
                logging.warning(f"Gemini audio {model}: {e}")
        return None
    except Exception as e:
        logging.error(f"ai_audio_req: {e}")
        return None
...
async def route_menu(message: types.Message, state: FSMContext):
    ...
    elif t == "🌐 Websayt": await show_website(message)
    ...
    elif t == "📝 So'z qo'shish":
        await cmd_soz_qosh(message, state)
    elif t == "📋 So'zlar ro'yxati":
        await cmd_sozlar(message)
    else:
        await message.answer("🏠 Asosiy menyu", reply_markup=main_menu(uid))
...
async def cmd_start(message: types.Message):
    ...
    kb_rows = [[InlineKeyboardButton(text="👥 Botni guruhga qo'shish", url=add_url)]]
    if WEBSITE_URL:
        kb_rows.append([InlineKeyboardButton(text="🌐 Websayt va mini o'yinlar", url=WEBSITE_URL)])
    ...
    await message.answer("👇 Guruhingizga qo'shing yoki websaytda mini o'yinlarni sinab ko'ring:", reply_markup=kb)
...
@dp.message(F.text == "🌐 Websayt")
async def show_website(message: types.Message):
    if not WEBSITE_URL:
        await message.answer("🌐 Websayt hozircha sozlanmagan."); return
    base = WEBSITE_URL.rstrip('/')
    kb = InlineKeyboardMarkup(inline_keyboard=[
        [InlineKeyboardButton(text="🌐 Saytni ochish", url=WEBSITE_URL)],
        [InlineKeyboardButton(text="👤 Profilim", url=f"{base}/profile.html"),
         InlineKeyboardButton(text="🏆 Reyting", url=f"{base}/rating.html")],
        [InlineKeyboardButton(text="🛍️ Magazin", url=f"{base}/shop.html"),
         InlineKeyboardButton(text="🔍 Qidiruv", url=f"{base}/search.html")],
        [InlineKeyboardButton(text="🎮 Mini o'yinlar", url=f"{base}/games.html")]])
    await message.answer(
        "🌐 <b>BilimChallenge Websayt</b>\n\n"
        "Saytda botning veb-versiyasi bor — savollarga bevosita brauzerda javob bering:\n\n"
        "👤 Profilingiz — coin, streak, liga, duel statistikasi\n"
        "🏆 Global reyting\n"
        "🛍️ Magazin — coinlarga profil bezaklari\n"
        "🔍 Qidiruv — boshqa o'yinchilarni toping\n"
        "🎮 4 ta mini o'yin — kichik coin mukofoti bilan\n\n"
        "Telegram orqali bir marta kirasiz — coinlaringiz bot bilan bir xil bo'ladi.",
        parse_mode="HTML", reply_markup=kb)
...
async def show_help(message: types.Message):
    me = await bot.get_me()
    url = f"https://t.me/{me.username}?startgroup=start"
    kb_rows = [[InlineKeyboardButton(text="👥 Guruhga qo'shish", url=url)]]
    if WEBSITE_URL:
        kb_rows.append([InlineKeyboardButton(text="🌐 Websayt va mini o'yinlar", url=WEBSITE_URL)])
    kb = InlineKeyboardMarkup(inline_keyboard=kb_rows)
    await message.answer(
...
async def handle_ielts(message: types.Message, state: FSMContext):
    ...
    if section == "reading":
        ...
        result = f"\U0001f4d6 <b>Reading natijasi</b>\n\n\u2705 {cc}/{total}  \U0001f4b0 +{earned}"
        if cc < total:
            wrong = [a for a in cls if a not in uas]
            result += "\n\u274c To\'g\'ri javoblar:\n" + "\n".join(f"\u2022 {w}" for w in wrong)
        await message.answer(result, parse_mode="HTML",
            reply_markup=InlineKeyboardMarkup(inline_keyboard=[[
                InlineKeyboardButton(text="\u27a1\ufe0f Keyingi", callback_data=f"ielts_{section}")]]))
        await state.clear()
        return
    ...
    if message.text and is_lazy(message.text):
        ...
        await message.answer("\u274c 0 coin \u2014 sifatsiz javob!\nTo\'liq javob yozing.",
            reply_markup=InlineKeyboardMarkup(inline_keyboard=[[
                InlineKeyboardButton(text="\U0001f504 Qayta", callback_data=f"ielts_{section}")]]))
        return
    ...
    if section == "essay":
        ...
        if message.voice:
            await message.answer("\u23f3 Audio tahlil qilinmoqda...")
            prompt = (f"Siz IELTS {section.capitalize()} baholovchisisiz. Ovozli xabarni tinglang va "
                      f"O\'ZBEK TILIDA baholang: talaffuz, ravonlik, grammatika, so\'z boyligi.\n"
                      f"Band Score: X.X/9.0\nKuchli va zaif tomonlar.\nFaqat: BAND:X.X")
            analysis = await ai_audio_req(message.voice.file_id, prompt)
            if not analysis:
                analysis = "\u26a0\ufe0f Audio tahlil qilib bo\'lmadi. Matn shaklida qayta yuboring."
        elif message.audio:
            await message.answer("\u23f3 Audio tahlil qilinmoqda...")
            prompt = (f"Siz IELTS {section.capitalize()} baholovchisisiz. Audio faylni tinglang va "
                      f"O\'ZBEK TILIDA baholang: talaffuz, ravonlik, grammatika, so\'z boyligi.\n"
                      f"Band Score: X.X/9.0\nKuchli va zaif tomonlar.\nFaqat: BAND:X.X")
            analysis = await ai_audio_req(message.audio.file_id, prompt)
            if not analysis:
                analysis = "\u26a0\ufe0f Audio tahlil qilib bo\'lmadi. Matn shaklida qayta yuboring."
        elif message.text:
            ...
            await message.answer("\U0001f3a4 Ovozli xabar, audio yoki matn yuboring!")
            return
    else:
        await message.answer("\u2753 Noma\'lum bo\'lim.")
        return

    if not analysis:
        analysis = "\u26a0\ufe0f AI hozirda ishlamayapti."
    ...
    score_text = f"\n\U0001f3af Band: <b>{band}/9.0</b>" if band else ""
    await send_long(message.chat.id,
        f"\U0001f916 <b>AI Tahlil:</b>\n\n{analysis}{score_text}\n\n\U0001f4b0 +{earned} coin (max {coins})",
        parse_mode="HTML")
...
async def duel_challenge(callback: types.CallbackQuery):
    ...
    try:
        kb_op = InlineKeyboardMarkup(inline_keyboard=[[
            InlineKeyboardButton(text="✅ Qabul qilish", callback_data=f"duel_accept_{d_id}"),
            InlineKeyboardButton(text="❌ Rad etish", callback_data=f"duel_decline_{d_id}")]])
        await bot.send_message(op_id,
            f"⚔️ <b>{ch_name}</b> sizni duelga chaqirdi!\n"
            f"💰 Tikish: <b>{bet} coin</b>  ❓ 5 ta savol\n\n"
            f"Qabul qilasizmi?",
            parse_mode="HTML", reply_markup=kb_op)
    except:
...
async def duel_accept(callback: types.CallbackQuery):
    ...
    # Challenger ga ham tasdiq so'rash
    ch_name_t = ch[2] if ch else str(ch_id)
    op_name_t = op[2] if op else str(op_id)
    try:
        kb_ch = InlineKeyboardMarkup(inline_keyboard=[[
            InlineKeyboardButton(text="✅ Ha, tayyor!", callback_data=f"duel_ready_{d_id}"),
            InlineKeyboardButton(text="❌ Bekor qilish", callback_data=f"duel_cancel_{d_id}")]])
        await bot.send_message(ch_id,
            f"⚔️ <b>{op_name_t}</b> duelni qabul qildi!\n"
            f"💰 Tikish: <b>{bet} coin</b>\n\n"
            f"Siz ham tayyormisiz?",
            parse_mode="HTML", reply_markup=kb_ch)
    except Exception as e:
        logging.error(f"duel ready ch: {e}")
    try: await callback.message.edit_text(
        f"✅ Tasdiqladingiz!\n⏳ <b>{ch_name_t}</b> tayyor bo'lishini kuting...",
        parse_mode="HTML")
    except: pass
...
@dp.callback_query(F.data.startswith("duel_ready_"))
async def duel_ready_cb(callback: types.CallbackQuery):
    """Challenger duelga tayyor deydi - o'yin boshlanadi"""
    d_id = int(callback.data[11:])
    duel = db.get_duel(d_id)
    if not duel:
        await callback.answer("Duel topilmadi!", show_alert=True); return
    _, ch_id, op_id, bet, status, *_ = duel
    if callback.from_user.id != ch_id:
        await callback.answer("Bu sizning duelingiz emas!", show_alert=True); return
    if status == "active":
        await callback.answer("Duel allaqachon boshlangan!", show_alert=True); return
    ...
    start_text = (f"\u2694\ufe0f <b>DUEL BOSHLANDI!</b>\n\n"
                  f"\U0001f464 {ch_name} vs {op_name}\n"
                  f"\U0001f4b0 Tikish: {bet} coin  \u2753 {len(q_ids)} savol\n"
                  f"\U0001f3c6 G'olib 2x coin oladi!")
    try: await bot.send_message(ch_id, start_text, parse_mode="HTML")
    except: pass
    try: await bot.send_message(op_id, start_text, parse_mode="HTML")
    except: pass
...
async def check_duel_end(d_id):
    ...
    if winner_id:
        ...
        try: await bot.send_sticker(winner_id, sticker=STICKER_CORRECT)
        except: pass
    else:
        db.add_coins(ch_id, bet)
        db.add_coins(op_id, bet)
...
async def handle_group_msg(message: types.Message):
    ...
    # Guruhda faqat so'z o'yini — savol-javob tizimi yo'q
...
@dp.message(F.text == "📋 So'zlar ro'yxati")
