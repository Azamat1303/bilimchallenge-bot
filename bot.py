...
async def groq_analyze(prompt: str) -> str:
    try:
        ...
        async with aiohttp.ClientSession() as session:
            async with session.post(url, headers={"Content-Type": "application/json"}, json={"contents": [{"parts": [{"text": prompt}]}]}, timeout=aiohttp.ClientTimeout(total=30)) as resp:
    ...
    try:
        async with aiohttp.ClientSession() as session:
            async with session.post("https://api.groq.com/openai/v1/chat/completions", headers={"Authorization": f"Bearer {GROQ_API_KEY}", "Content-Type": "application/json"}, json={"model": GROQ_MODEL, "messages": [{"role": "user", "content": prompt}], "max_tokens": 1000}, timeout=aiohttp.ClientTimeout(total=30)) as resp:
                data = await resp.json()
                return data["choices"][0]["message"]["content"]
    except Exception as e:
        return "⚠️ AI hozirda ishlamayapti. Keyinroq urinib ko'ring."
...
async def handle_ai_chat(message: types.Message, state: FSMContext):
    ...
    ai_reply = None
    full_prompt = "Siz BilimChallenge botining yordamchi AI sisiz. O'zbek tilida qisqa, foydali va do'stona javob bering.\n\n"
    ...
    try:
        url = f"https://generativelanguage.googleapis.com/v1beta/models/gemini-1.5-flash:generateContent?key={GEMINI_API_KEY}"
        async with aiohttp.ClientSession() as session:
            async with session.post(url, headers={"Content-Type": "application/json"}, json={"contents": [{"parts": [{"text": full_prompt}]}]}, timeout=aiohttp.ClientTimeout(total=30)) as resp:
                data = await resp.json()
                if "candidates" in data and data["candidates"]:
    ...
    if not ai_reply:
        try:
            async with aiohttp.ClientSession() as session:
                msgs = [{"role": "system", "content": "O'zbek tilida qisqa javob ber."}] + chat_history[-6:] + [{"role": "user", "content": user_text}]
                async with session.post("https://api.groq.com/openai/v1/chat/completions", headers={"Authorization": f"Bearer {GROQ_API_KEY}", "Content-Type": "application/json"}, json={"model": GROQ_MODEL, "messages": msgs, "max_tokens": 800}, timeout=aiohttp.ClientTimeout(total=30)) as resp:
                    data = await resp.json()
                    ai_reply = data["choices"][0]["message"]["content"]
        except Exception as e:
            await message.answer("⚠️ AI hozirda ishlamayapti.")
            return
